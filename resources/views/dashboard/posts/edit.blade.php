...
@section('breadcrumb')
    ...
    <span class="text-gray-700 dark:text-gray-300">Sửa</span>
@endsection

@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Sửa tin đăng</h1>
    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Cập nhật thông tin bất động sản</p>
</div>

<form action="{{ route('dashboard.posts.update', $post->id) }}" method="POST" class="space-y-6">
    @csrf
    @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="5"
                                  ...
                                  placeholder="Mô tả chi tiết về bất động sản...">{{ old('description', $post->description) }}</textarea>
                        @error('description')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="raw_content" rows="3"
                                  ...
                                  placeholder="Nội dung gốc từ nguồn...">{{ old('raw_content', $post->raw_content) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="price_text" value="{{ old('price_text', $post->price_text) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="price_value" value="{{ old('price_value', $post->price_value) }}"
                               class="w-full px-4 py-2.5 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg text-sm focus:ring-2 focus:ring-teal-500"
                    ...
                    <div>
                        ...
                        <input type="text" name="area_text" value="{{ old('area_text', $post->area_text) }}"
                               class="w-full px-4 py-2.5 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg text-sm focus:ring-2 focus:ring-teal-500"
                               placeholder="VD: 80m2">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="area_value" value="{{ old('area_value', $post->area_value) }}"
                               class="w-full px-4 py-2.5 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg text-sm focus:ring-2 focus:ring-teal-500"
                               placeholder="80">
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Số mặt tiền</label>
                        <input type="text" name="depth_text" value="{{ old('depth_text', $post->frontage_count) }}"
                               class="w-full px-4 py-2.5 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg text-sm focus:ring-2 focus:ring-teal-500"
                               placeholder="VD: 1, 2, 3, 4 mặt tiền">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Mặt tiền (m)</label>
                        <input type="text" name="frontage_texts" value="{{ old('frontage_texts', is_array($post->frontage_texts) ? implode(',', $post->frontage_texts) : '') }}"
                               class="w-full px-4 py-2.5 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg text-sm focus:ring-2 focus:ring-teal-500"
                               placeholder="VD: 5m">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="depth_text" value="{{ old('depth_text', $post->depth_text) }}"
                               class="w-full px-4 py-2.5 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg text-sm focus:ring-2 focus:ring-teal-500"
                               placeholder="VD: 20m">
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="address_text" value="{{ old('address_text', $post->address_text) }}"
                               class="w-full px-4 py-2.5 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg text-sm focus:ring-2 focus:ring-teal-500"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="text" name="ward" value="{{ old('ward', $post->ward) }}"
                                   class="w-full px-4 py-2.5 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg text-sm focus:ring-2 focus:ring-teal-500"
                        ...
                        <div>
                            ...
                            <input type="text" name="district" value="{{ old('district', $post->district) }}"
                                   class="w-full px-4 py-2.5 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg text-sm focus:ring-2 focus:ring-teal-500"
                                   placeholder="Quận 7">
                        </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="property_type" class="w-full px-4 py-2.5 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg text-sm">
                            ...
                            <option value="Nhà phố/Nhà riêng" {{ old('property_type', $post->property_type) == 'Nhà phố/Nhà riêng' ? 'selected' : '' }}>Nhà phố/Nhà riêng</option>
                            <option value="Căn hộ chung cư" {{ old('property_type', $post->property_type) == 'Căn hộ chung cư' ? 'selected' : '' }}>Căn hộ chung cư</option>
                            <option value="Biệt thự (Villa)" {{ old('property_type', $post->property_type) == 'Biệt thự (Villa)' ? 'selected' : '' }}>Biệt thự</option>
                            <option value="Đất nền/Đất thổ cư" {{ old('property_type', $post->property_type) == 'Đất nền/Đất thổ cư' ? 'selected' : '' }}>Đất nền</option>
                        </select>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="url" name="facebook_url" value="{{ old('facebook_url', $post->facebook_url) }}"
                               class="w-full px-4 py-2.5 bg-gray-100 dark:bg-gray-700 border-0 rounded-lg text-sm focus:ring-2 focus:ring-teal-500"
